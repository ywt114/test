...
sed -i '/$(eval \$(call BuildPackage,\$(PKG_NAME)))/i \PKG_FETCH_CMD:=curl --connect-timeout 20 --retry 5 --location -o $(DL_DIR)/$(PKG_SOURCE) $(PKG_SOURCE_URL)$(PKG_SOURCE) || exit 1\n' package/network/services/ddnsto/Makefile
sed -i '/$(eval \$(call BuildPackage,\$(PKG_NAME)))/i \PKG_FETCH_CMD:=curl --connect-timeout 20 --retry 5 --location -o $(DL_DIR)/$(PKG_SOURCE) $(PKG_SOURCE_URL)$(PKG_SOURCE) || exit 1\n' package/network/services/istoreenhance/Makefile
sed -i '/$(eval \$(call BuildPackage,\$(PKG_NAME)))/i \PKG_FETCH_CMD:=curl --connect-timeout 20 --retry 5 --location -o $(DL_DIR)/$(PKG_SOURCE) $(PKG_SOURCE_URL)$(PKG_SOURCE) || exit 1\n' package/network/services/linkease/Makefile
sed -i '/$(eval \$(call BuildPackage,\$(PKG_NAME)))/i \PKG_FETCH_CMD:=curl --connect-timeout 20 --retry 5 --location -o $(DL_DIR)/$(PKG_SOURCE) $(PKG_SOURCE_URL)$(PKG_SOURCE) || exit 1\n' package/network/services/linkmount/Makefile
